...
test.group('Array node', () => {
  ...
  test('compute wildcard path to nested arrays', async ({ assert }) => {
    ...
    assert.assertFormatted(compiledOutput, [
      ...
      `  meta: meta,`,
      `  fieldName: 'contacts',`,
      `  fieldPath: root_item_i + '.' + 'contacts',`,
      ...
      `  fieldPath: root_item_i + '.' + 'contacts' + '.' + contacts_3_i,`,
      `  wildCardPath: '*.contacts.*',`,
      `  mutate: defineValue,`,
      `  report: report,`,
      `  isValid: true,`,
      ...
      `out[root_item_i]['contacts'][contacts_3_i] = contacts_3_item.value;`,
      `}`,
      `}`,
      `}`,
      `}`,
