...
test.group('Array node', () => {
  ...
  test('compute wildcard path to nested arrays', async ({ assert }) => {
    ...
    assert.assertFormatted(compiledOutput, [
      ...
      '});',
      `ensureExists(root_item_item);`,
      `root_item_item.isValidDataType = ensureIsObject(root_item_item);`,
      `if (root_item_item.isValidDataType) {`,
      ...
      `contacts_3.isValidDataType = ensureIsArray(contacts_3);`,
      `if (contacts_3.isValidDataType) {`,
  ...
  test('define array field name with special characters', async ({ assert }) => {
    ...
    assert.assertFormatted(compiledOutput, [
      ...
      `primaryContacts_3.isValidDataType = ensureIsArray(primaryContacts_3);`,
      `if (primaryContacts_3.isValidDataType) {`,
  ...
  test('create JS output with implicit array validations', async ({ assert }) => {
    const compiler = new Compiler({
      type: 'root',
      schema: {
        type: 'array',
