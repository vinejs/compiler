...
test.group('Tuple node', () => {
  test('create JS output for tuple node', async ({ assert }) => {
    ...
    assert.assertFormatted(compiledOutput, [
      ...
      `const root_item_is_array = ensureIsArray(root_item);`,
      `if (root_item.isValid && root_item_is_array) {`,
      `refs['ref://2'].validator(root_item.value, refs['ref://2'].options, root_item);`,
      `}`,
      `if (root_item_is_array) {`,
  ...
  test('create JS output for tuple node with implicit tuple validations', async ({ assert }) => {
    const compiler = new Compiler({
      type: 'root',
      schema: {
        type: 'tuple',
    ...
    assert.assertFormatted(compiledOutput, [
      ...
      `}`,
      `if (root_item.isValid && root_item_is_array) {`,
      `  refs['ref://3'].validator(root_item.value, refs['ref://3'].options, root_item);`,
      `}`,
      `if (root_item.isValid) {`,
